                    ...
                    @if ($project->id_int == $interface->id_int)
                        @if($project->id_mode == $mode->id_mode)
                            <tr>
                                ...
                                <form id="confidentForm" method="post" id="prjName"
                                      action="{{url(route('projects_project',['id_prj' => $project]))}}"
                                      enctype="multipart/form-data">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <!-- Name of the project -->
                                    <th id="name-cell">
                                        <button class="buttonList" type="submit"
                                                class="project-details">{{$project->name_prj}}</button>
                                    </th>

                                    <!-- Limitation mode of the project ( and value ) -->
                                    @if($project->id_mode == 1)
                                        <th>{{$interface->label_int}} <br> {{$mode->label_mode}}
                                            ({{$project->limit_prj}} minutes)
                                        </th>
                                    @else
                                        <th>{{$interface->label_int}} <br> {{$mode->label_mode}}
                                            ({{$project->limit_prj}} annotations)
                                        </th>
                                    @endif

                                <!-- Project data export button -->
                                    <th><a href="{{url(route('projects_project_export',['id_prj' => $project]))}}">Export</a>
                                    </th>
                                    ...
                                    @foreach($particitionProject as $aPart)
                                        @if($aPart->id_prj == $project->id_prj)
                                            <th>
                                                <p class="interval">Expert project<br>confidence level :<br> <input
                                                        class="intervalle_input" id="numberconf" name="confiance"
                                                        type="range" min="0" max="2" step="1"
                                                        value="{{$aPart->expert_project_confidence_level}}">
                                                    <div id="trust2">
                                                <p id="doubt2">doubt</p>
                                                <p id="confident2">confident</p>
                                                <p id="very_confident2">highly confident</p></div></p>
                                            </th>
                                        @endif
                                    @endforeach
